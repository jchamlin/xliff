...
def is_element_effectively_empty(elem):
    if elem is None:
        return True
    ...
    for child in elem.iter():
        if child is not elem:
            if child.tag is not None or (child.text and child.text.strip()):
                return False
    return True
    ...
    for lang, root in parsed_files.items():
        ...
        for unit in root.findall(".//x:file/x:unit", namespaces=XLIFF_NS):
            ...
            for seg in unit.findall(".//x:segment", namespaces=XLIFF_NS):
                ...
                elif is_element_effectively_empty(targets[0]):
                    issues[lang].append(f"<target> in unit '{uid}' is empty")
                if sources and is_element_effectively_empty(sources[0]):
