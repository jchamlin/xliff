...
# STEP 0: BOM Consistency Check
def check_bom_consistency(file_paths):
    result = {}
    boms = {}
    for lang, path in file_paths.items():
        with open(path, "rb") as f:
            start_bytes = f.read(4)
            has_bom = start_bytes.startswith(b'\xef\xbb\xbf')
            boms[lang] = has_bom
            result[lang] = "✅ BOM Present" if has_bom else "✅ No BOM"
    if len(set(boms.values())) > 1:
        result["Mismatch"] = "❌ BOM mismatch between files"
    else:
        result["Consistency"] = "✅ BOM consistent across files"
    return result

XLIFF_NS = {'x': 'urn:oasis:names:tc:xliff:document:2.0'}


# STEP 0A: Fast line-by-line structure validation (indentation + tag match)
def check_line_structure_match(file1_path, file2_path):
    ...
    structure1 = analyze(file1_path)
    structure2 = analyze(file2_path)
...
def filtered_untranslated_check(parsed):
    untranslated = []
    for lang, root in parsed.items():
        for unit in root.findall(".//{urn:oasis:names:tc:xliff:document:2.0}unit"):
            seg = unit.find(".//{urn:oasis:names:tc:xliff:document:2.0}segment")
            target = seg.find("{urn:oasis:names:tc:xliff:document:2.0}target") if seg is not None else None
            if target is None:
                untranslated.append(unit.attrib.get("id"))
                continue
            text_content = "".join(target.itertext()).strip()
            if not text_content:
                untranslated.append(unit.attrib.get("id"))
    return untranslated
